        ...
        <v-tabs v-model="active" color="cyan" dark slider-color="yellow">
            ...
            <v-tab-item :key="0">
                ...
                >
                    <template v-slot:item.isPublished="{item}">
                        <v-checkbox disabled v-model="item.isPublished"></v-checkbox>
                    </template>
                    <template v-slot:item.action="{item}">
                        <v-icon small v-on:click="showEditPost(item.id)">mdi-pencil</v-icon>
                        <v-icon small v-on:click="showConfirmation(item.id)">mdi-delete</v-icon>
                    </template>
                </v-data-table>
            ...
            <v-tab-item :key="1">
                ...
                >
                    ...
                    <template v-slot:item.action="{item}">
                        <v-btn small flat color="success" v-on:click="showEditPost(item.id)">Edit</v-btn>
                        <v-btn
                            small
                            flat
                            color="error"
                            v-on:click="showConfirmation(item.id)"
                        >Delete</v-btn>
                    </template>
                    <div class="text-xs-center">
                        <v-pagination
        ...
        <v-dialog v-model="edit.show" max-width="290">
            <v-card>
                ...
                <v-card-actions>
                    ...
                    <v-btn color="green darken-1" flat="flat" @click="edit.show = false">Cancel</v-btn>

                    <v-btn color="green darken-1" flat="flat" @click="editPost(edit.post)">Save</v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
